...
- **Enemies** — Hawkers patrol back and forth and die in one hit. Okadas (from stage ~3) are faster ground hazards that die just as easily but are harder to react to. Mosquitoes (from stage ~6) hover in the air in a small bobbing pattern instead of walking a lane, so you generally have to jump or throw to reach them. Agberos are tougher (2 hits), block the path in a "traffic gridlock" barricade near the end of most stages, and always have a low bridge nearby so you can hop over them entirely instead of fighting. Enemies get faster on later stages.
- **The boss** — Stage 10 ends with Chief Agbero, a bigger, 5-hit boss who throws bottles at you on a ~2.2s timer (watch for the health bar at the top of the screen). Unlike the regular barricades, there's no bypass bridge here — you have to actually fight him to reach the final Danfo.
...
### Enemy variety and the boss fight

The original version only had two enemy kinds (Hawker, Agbero), which the "known limitations" list used to flag as thin. It's now five: Hawker and Okada are both ground patrollers (`Enemy` handles them identically — same `moveAndCollide`, same wall/ledge-bounce patrol — they just differ in `ENEMY_SPEED`/`ENEMY_SIZE`), Mosquito is the odd one out (a `flying: boolean` flag on `Enemy` skips gravity and tile collision entirely and instead moves on a Lissajous-ish sine curve around a fixed origin point), and Agbero/Boss share a `hit()`/health/squish-on-death lifecycle with everything else.

A few things worth knowing if you extend this:
- **New kinds unlock gradually**, gated by the same `t` (0..1 difficulty ramp) the rest of `Level.ts` uses — `OKADA_UNLOCK_T = 0.2`, `MOSQUITO_UNLOCK_T = 0.45`. Bump a threshold down to introduce something earlier, or add a new constant the same way for a 6th enemy.
- **The boss (`kind: "boss"`) is just an `Enemy`** with higher health (5, vs. 1-2 for everything else) and a bigger sprite. Rather than hand-authoring a new 20-ish-row pixel grid, `CHIEF_AGBERO` is a copy of the regular `AGBERO` matrix with the cap/vest characters swapped for gold/purple ones, drawn at 1.5x scale via a `scale` parameter added to `drawSprite`. If you want a visually distinct boss shape rather than "bigger Agbero," that's the thing to actually redraw.
- **The boss fights back.** `GameEngine.updateBossAttack()` finds the (single, currently) alive boss enemy each frame and, on a fixed interval (`BOSS_ATTACK_INTERVAL_SECONDS`), spawns a `Projectile` aimed at wherever the player currently is. This reuses the exact same `Projectile` class as the player's slipper throw, distinguished by an `owner: "player" | "boss"` field — collision handling is split into `handleProjectileEnemyCollisions` (player projectiles vs. enemies) and `handleBossProjectilePlayerCollisions` (boss projectiles vs. the player) so the two can never accidentally hurt the wrong side. The boss's bottle also gets a bit of gravity (`owner === "boss"` gives it half of normal `GRAVITY`) so it arcs instead of flying flat like the slipper — it's meant to be dodged by jumping or repositioning, not just outrun.
- **The boss arena has no bypass bridge**, unlike every regular Agbero gate. `Level.ts`'s `placeBossArena` is a separate function from `placeAgberoGate` for exactly this reason — a boss is meant to be fought, and accidentally reusing the bypass-bridge logic for it would let players skip the fight entirely. It only gets called for the guaranteed final gate when `stage === STAGE_COUNT`; every earlier gate on stage 10 (there can be up to 2 extra, from the normal difficulty scaling) is still a regular skippable Agbero.
- **The boss health bar is screen-fixed, not world-fixed** (`drawBossHealthBar`, drawn last in `render()` so nothing overlaps it) — deliberately, so it reads like a classic boss bar pinned to the top of the screen regardless of camera scroll, rather than floating in the world above the boss's head like the enemy hit-flash or the throw-cooldown meter do.

If you want to add a second boss later (say, at stage 5 as a mid-boss), the pattern to copy is: add an `EnemyKind`, give it stats in the three `Record<EnemyKind, ...>` tables in `Entities.ts`, decide whether it needs a `placeXArena`-style dedicated placement function or fits into the normal patrol/gate flow, and decide whether it needs its own attack pattern or can reuse `updateBossAttack`'s.
...
- **The stage-10 boss is a single, fixed fight** — Chief Agbero doesn't scale or vary, and there's only one boss in the whole game. If the difficulty curve ever gets a second pass, a mid-run boss (stage 5, say) using the same pattern described above would probably matter more than tuning the existing one further.
- Ideas that were discussed but intentionally deferred: a leaderboard (would need a real backend — currently `localStorage` only, so high scores are per-browser; explicitly parked until/unless it's actually needed), and real recorded music/SFX as an alternative to the synthesized audio if the chiptune aesthetic ever needs to grow up.
